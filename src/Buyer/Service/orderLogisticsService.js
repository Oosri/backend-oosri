...
function buildHaulamShipmentPayload({ deliveryAddress, buyer, orderItems, productMap, declaredValue, selectedServiceType }) {
...
function buildShipmentRequest({ provider, deliveryAddress, buyer, orderItems, productMap, declaredValue, selectedServiceType }) {
  if (provider === 'HAULAM') {
...
module.exports.processOrdersLogistics = async ({ orderIds, buyerId, paymentIntentId }) => {
  ...
  const requestPayload = buildShipmentRequest({
    ...
    productMap,
    declaredValue,
    selectedServiceType: orders[0]?.shippingServiceCode || null,
  });
