...
const svc = createNotificationService(SellerNotification, 'sellerId');

module.exports.getNotifications = async (req, res) => {
  try {
    const { skip = 0, limit = 20 } = req.query;
    const data = await svc.getAll({ ownerId: req.seller._id, skip, limit });
...
module.exports.markRead = async (req, res) => {
  try {
    const updated = await svc.markRead({ ownerId: req.seller._id, notificationId: req.params.id });
...
module.exports.markAllRead = async (req, res) => {
  try {
    await svc.markAllRead({ ownerId: req.seller._id });
...
module.exports.deleteNotification = async (req, res) => {
  try {
    await svc.deleteOne({ ownerId: req.seller._id, notificationId: req.params.id });
